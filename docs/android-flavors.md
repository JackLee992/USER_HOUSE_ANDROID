...
1.2 已在同一真机完成 content1 的同签名覆盖、五键原始存档比较、37 款启动与四款实际操作。后续真实触控检查发现 Gecko 宿主没有实现 `onChoicePrompt`，导致 HTML 语言/主题选择器只能获得焦点，不能弹出菜单；先前 DOM 设置选项的语言检查不能覆盖此故障。现已补原生单选/多选对话框，并在新原生 + content2 测试包上完成语言、主题、取消、后台冷启动与三消模式选择器的 21 项真实触控检查。旧 content2 没有性能选择器，该项明确跳过，不能当作通过。

content3已通过真实GitHub下载46包、43,301,194字节的原子激活，35包复用，五键存档逐字节保留；37个独立目录图标已在手机显示。最新设置29项真实触控检查无skip，涵盖三档性能、语言/主题与三消确认。首页12次原生Android上下滑动全部有效；Gecko Marionette合成滑动不一定驱动APZ，不能把自动化0位移当作实际用户无法滑动。普通档泡泡完成静止和3次真实发射短测，调度约59.6/58.5Hz，非GPU/功耗或所有游戏流畅保证。

最终content3已通过真实触控回退至完整缓存content2，原五键字节一致；随后覆盖正式1.2.0，自动采用较新的内置content3。正式版实际SAF导出确认四项游戏数据结构一致，settings只有进入设置页和备份过滤空API字段造成的预期变化。手机保留正式兼容版，测试调试YAML与端口转发已清理。详细证据和限制见 [1.2 验证记录](android-validation-1.2.md)。以上 1.1 基线不代替 1.2 完整验收。1.0 历史记录见 [历史验证](android-validation.md)。
