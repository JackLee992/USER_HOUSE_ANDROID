...
export async function initWanbanXiaowu(options = {}) {
  ...
  const GAME_RULES = {
    ...
    screwclassic: '完整保留我们 fork 时基线版本的拧螺丝玩法。普通模式每局随机生成42到47块分层面板，清空全部面板即通关；无尽模式每层生成28到32块，剩余螺丝不超过24颗或面板不超过8块时自动补入下一层，不弹出关卡结算。上层会遮挡下层螺丝；板件剩一颗螺丝时绕固定点摆动，最后一颗拆除后受重力加速下落，并会被前层仍固定的螺丝挡住。顶部同时开启3个颜色工具盒，同色3颗自动打包；非当前颜色进入5格临时托盘，可增加盒子到最多6个。',
  ...
  function gameLogFieldRules(game, roleName) {
    ...
    if (game === 'screwclassic') return '字段说明：拧螺丝完整保留我们 fork 时基线版本的普通与无尽玩法；普通模式每局42到47块板件，无尽模式每层28到32块并在接近清空时连续补层。默认3个颜色工具盒，可扩到6盒；板件剩一颗螺丝时绕固定点摆动，最后一颗拆除后受重力下落。';
...
.wb-xq-river { grid-column:1 / -1; grid-row:6; z-index:1; min-height:24px; display:flex; align-items:center; justify-content:space-around; pointer-events:none; font-weight:1000; letter-spacing:.35em; color:rgba(120,53,15,.58); background:linear-gradient(90deg,transparent,rgba(255,247,220,.72),transparent); }
      ...
      #${POPUP_ID}.wb-mono .wb-screw-slot span { filter:none; image-rendering:auto; }
