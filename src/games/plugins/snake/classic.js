...
function createSurface(env){
  ...
  const root=doc.createElement('section');root.id='wb-snake-classic-fullscreen';root.className='snake-classic';root.setAttribute('aria-label','经典贪吃蛇');root.innerHTML=`
    ...
    <main class="sc-stage"><canvas class="sc-canvas" width="840" height="840" aria-label="经典贪吃蛇棋盘"></canvas><div class="sc-ready" data-classic-ready><strong>准备好了吗</strong><span>滑动棋盘或按方向键开始</span></div></main>
    ...
    <div class="sc-mask" data-classic-mask hidden><div class="sc-dialog"><h2 data-classic-dialog-title>已暂停</h2><p data-classic-dialog-text>经典模式会保存当前棋盘和方向。</p><div class="sc-dialog-actions"><button class="primary" data-classic-resume type="button">继续</button><button data-classic-restart type="button">重新开始</button><button data-classic-exit-dialog type="button">保存并退出</button></div></div></div>`;
...
export function createClassicGame(env, state) {
  ...
  function delay(){return Math.max(MIN_DELAY,INITIAL_DELAY-Math.floor(score/10)*SPEED_STEP);}
  function statePayload(){return {snake:snake.map(clonePoint),dir:{...dir},next:queued[0]||next,food:{...food},score,ready,turnCount,lastTurnSpeakAt,controlsHidden:true,controlMode:'classic-fullscreen',details:{...snakeStats}};}
  ...
  function draw(force=false){
    ...
    scoreLabel.textContent=String(score);speedLabel.textContent='速度 '+(1+Math.floor(score/40))+' · 长度 '+snake.length;syncPause();syncReady();
  }
