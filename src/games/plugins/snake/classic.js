...
export function createClassicGame(env, state) {
  function startSnake(state) {
    ...
    let dir = state?.dir || {x:1,y:0}, next = state?.next || dir, food = validFood(state?.food) ? state.food : randFood(), score = state?.score || 0, dead = false, destroyed = false;
    ...
    function randFood(){ const spots=[]; for(let y=2;y<n-2;y++) for(let x=2;x<n-2;x++) if(!snake.some(s=>s.x===x&&s.y===y)) spots.push({x,y}); return spots.length ? spots[Math.floor(Math.random()*spots.length)] : {x:10,y:10}; }
	    function save(){ if (!dead && !destroyed) env.saveProgress('snake', { snake, dir, next, food, score, turnCount, lastTurnSpeakAt, controlsHidden: controlMode !== 'keys', controlMode, details:snakeStats }); }
    ...
    const keyHandler = env.getHostDocument().onkeydown = e => { const k={ArrowUp:'up',ArrowDown:'down',ArrowLeft:'left',ArrowRight:'right',w:'up',s:'down',a:'left',d:'right'}[e.key]; if(k){ setSnakeDir(k); e.preventDefault(); } };
    ...
    return {save, getState:()=>({mode:'classic',snake:snake.map(p=>({...p})),dir:{...dir},next:{...next},food:{...food},score,dead}), destroy(){destroyed=true;clearTimeout(env.snakeTimer);if(env.getHostDocument().onkeydown===keyHandler)env.getHostDocument().onkeydown=null;box.ontouchstart=box.ontouchend=box.ontouchcancel=null;}};
	  }
  return startSnake(state);
}
