...
test('continuous steering turns at a bounded rate and moves in real time',()=>{
  ...
  assert.ok(player.angle>0&&player.angle<Math.PI/2,'turning is smooth, not an instant reversal');
});
...
test('boost is faster, spends length, and automatically stops at the safe minimum',()=>{
  ...
  assert.ok(fast.state.snakes[0].x-normal.state.snakes[0].x>60);
...
test('food is consumed once and grows the player',()=>{
  ...
  arena.advance(1/60);assert.equal(player.length,178);assert.equal(player.eaten,1);assert.equal(arena.state.food.length,0);
  arena.advance(1/60);assert.equal(player.length,178);
});
