...
try{
  ...
  await click('#wb-zuma-pause');const paused=await state();await wait(700);const pausedAgain=await state();assert.deepEqual(pausedAgain,paused);checks.push('Pause freezes exact engine state');
...
}catch(error){await screenshot('failure');console.error(JSON.stringify({passed:false,checks,error:error.stack,errors},null,2));process.exitCode=1;}finally{socket.close();}
