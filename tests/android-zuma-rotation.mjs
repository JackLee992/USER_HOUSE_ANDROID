...
try {
 ...
 for(const [rotation,name]of[['0','portrait'],['1','landscape'],['0','portrait-restored']]){
  ...
  const l=await layout();assert.equal(l.view.layout,rotation==='1'?'landscape':'portrait');assert(l.view.paused);assert.deepEqual(await core(),before,'paused gameplay identity preserved across physical rotation');
  ...
  screenshot(`${out}/${name}.png`);checks.push({rotation:Number(rotation),name,layout:l,score:before.score,lives:before.lives,shots:before.details.shots,chainCount:before.chain.length});
 }
