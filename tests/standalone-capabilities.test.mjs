...
test('all seven offline themes and window preference survive restrictions', () => {
  assert.equal(STANDALONE_THEMES.length, 7);
  for (const [theme] of STANDALONE_THEMES) assert.equal(constrainStandaloneSettings({theme}).theme, theme);
...
test('old plugin backup imports only game data and strips unavailable capabilities', () => {
  ...
  assert.equal(plan[keys.STORAGE_SETTINGS].theme,'cyber');
...
test('Android back cancels a visible match-three dialog before leaving the game', () => {
  const back = source.slice(source.indexOf('  function standaloneBack()'),source.indexOf('\n  if (standalone) {\n    getHostDocument().body.classList.add'));
