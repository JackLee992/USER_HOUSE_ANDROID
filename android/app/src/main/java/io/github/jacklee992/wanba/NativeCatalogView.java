...
final class NativeCatalogView extends FrameLayout {
    ...
    NativeCatalogView(Context context, NativeIconLoader icons, Actions actions) {
        ...
        loading = text("玩吧",24,INK);loading.setTypeface(null,android.graphics.Typeface.BOLD);loading.setGravity(Gravity.CENTER);loading.setBackgroundColor(BG);
    ...
    private final class GlassNavigation extends FrameLayout {
        ...
        GlassNavigation(Context context){
            ...
            for(int i=0;i<3;i++){
                ...
                names[i]=text("",11,MUTED);names[i].setTypeface(android.graphics.Typeface.create("sans-serif-medium",android.graphics.Typeface.NORMAL));names[i].setGravity(Gravity.CENTER);names[i].setIncludeFontPadding(false);names[i].setMaxLines(1);names[i].setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);LayoutParams np=new LayoutParams(-1,dp(16),Gravity.TOP);np.topMargin=dp(39);item.addView(names[i],np);
