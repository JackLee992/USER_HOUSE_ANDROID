...
38 款小游戏装进一个可离线运行的 App。**1.3.1** 在 Android 和 iOS 上使用统一风格的原生游戏列表、我的收藏和设置，游戏代码、本地进度和记录仍共用。

**[下载可安装 APK](https://github.com/JackLee992/USER_HOUSE_ANDROID/releases/latest)** · **[iOS 内测与正式分发](docs/ios-distribution.md)** · **[原生界面设计](docs/native-shell-design.md)** · **[37 款游戏体验审计](docs/game-ux-audit-v1.md)**

## 安装与更新

1. Android 8.0 及以上设备，从 Release 选择安装包：系统内核较新的设备用 `wanba-system-1.3.0.apk`；系统内核较旧、无法更新 WebView 的手机用 `wanba-compat-1.3.0.apk`（内置 GeckoView，支持 ARM 64/32 位）。
...
游戏与美术更新：进入 Android 首页时自动检查，有更新才出现提示；也可在设置手动检查，查看变化包后下载并安装，存档继续保留。首页下拉刷新游戏列表。38 款游戏代码、美术与五语言各自维护版本，发布在 [USER_HOUSE_GAME_PACKS](https://github.com/JackLee992/USER_HOUSE_GAME_PACKS)。应用只拉取变化包，并在完整校验后切换。自动检查不在游戏中弹窗，不自动下载或安装。

数独 1.1.0 重做经典全屏界面，新增候选笔记、撤销/重做、逻辑说明提示、四档唯一解新题及数字优先输入。[竞品对标与实测截图](docs/sudoku-product-upgrade.md)

App / 内核更新：GitHub 版包含独立的「设置 → 检查更新」入口，首页也会检查新 App 版本。优先下载匹配的 APK 差分，重建并校验完整签名 APK 后交给 Android 确认安装；没有匹配差分时下载完整包。1.2.0/1.2.1 首次获得此模块需手动覆盖安装一次，**无需卸载**。
...
- 24 款单人游戏、14 款本地人机挑战；原版“拧螺丝”和升级版“疯狂拧螺丝”作为两个独立入口共存。首批 37 款的完整审计见 [体验审计](docs/game-ux-audit-v1.md)。
- 原生游戏列表支持收藏、我的、拖动排序和下拉刷新；统一浅色界面，取消主题选择。iOS 保留系统透明 Tab 拖动交互；Android 提供半透明浮动底栏和拖动切换。
- 贪吃蛇 1.1.0 新增离线 AI 竞技场、虚拟摇杆、消耗长度加速、食物成长与三分钟模式，保留旧经典模式。[玩法与边界](docs/snake-arena-upgrade.md)
- 俄罗斯方块 1.1.0 新增离线 AI 对战、马拉松、40 行冲刺，支持暂存、三块预览、落点、硬降、连击与垃圾行，横屏使用独立触控区，旧经典模式仍可继续。[玩法与边界](docs/tetris-battle-upgrade.md)
...
- 数独支持五语经典界面、3×3 笔记、自动候选及可选清理、整步撤销/重做、数字优先输入、提示说明和旧局恢复；祖玛 GPU 管理与数独按需绘制使用[共享游戏运行层](docs/shared-game-engine.md)。
- 首页 38 款游戏图标按玩法统一呈现，疯狂拧螺丝增加独立闪电标识；泡泡龙使用完整圆形球体素材，修正窄屏右侧裁切。
- 设置新增省电、普通、游戏三档；减少泡泡静止时的无效绘制，并优化首页滑动监听与装饰绘制。
- 两种 Android 版本与 iOS 均使用原生语言和性能模式选择器，覆盖取消与保存。
- 本地存档、个人记录、收藏和排序；JSON 备份导入/导出使用平台系统文件选择器。
...
五语言覆盖游戏目录、38 款规则说明、常见控件和动态状态；中文猜词内容、象棋文字和部分 Canvas/WASM 内绘制文字保留原样。它们不能仅通过页面语言包完全替换，具体范围见 [语言覆盖文档](docs/i18n-coverage-v1.md)。
...
```sh
cd android
./gradlew -PwanbaAppUpdater=true -PwanbaGameUpdates=true assembleSystemRelease assembleCompatRelease
```
...
需要 macOS、Xcode 和 XcodeGen。运行 `scripts/build-ios.sh build` 生成项目并构建模拟器 App；游戏资源从同一源码自动打包。iOS 使用 WKWebView，目录、设置、收藏、Tab 和文件面板使用 UIKit；当前只运行随包内容，不启用下载式游戏代码或 Android 更新器。设备签名、TestFlight、Ad Hoc 和 App Store 的完整步骤见 [分发指南](docs/ios-distribution.md)，实现与模拟器验证见 [iOS 支持](docs/ios-support.md)。
...
目录：`standalone/` 独立页面与能力配置，`src/` 共享游戏代码，`android/` 与 `ios/` 原生壳，`assets/app-brand/` 品牌素材，`docs/` 设计、审计和验证证据。
