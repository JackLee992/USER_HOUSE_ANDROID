...
37 款小游戏装进一个可离线运行的 App。**1.3.0** 在 Android 和 iOS 上使用统一风格的原生游戏列表、我的收藏和设置，游戏代码、本地进度和记录仍共用。iOS 当前提供可运行的模拟器工程；分发到用户设备需要开发者账号与签名。
...
游戏与美术更新：进入 Android 首页时自动检查，有更新才出现提示；也可在设置手动检查，查看变化包后下载并安装，存档继续保留。首页下拉刷新游戏列表。37 款游戏代码、美术与五语言各自维护版本，发布在 [USER_HOUSE_GAME_PACKS](https://github.com/JackLee992/USER_HOUSE_GAME_PACKS)。应用只拉取变化包，并在完整校验后切换。自动检查不在游戏中弹窗，不自动下载或安装。
